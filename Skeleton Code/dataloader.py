...
class CustomDataset(Dataset):
    ...
    def __init__(self, image_paths, target_paths, transform1, arg_mode=False,
                 arg_thres=0):  # initial logic happens like transform
    ...
    def __getitem__(self, index):
        ...
        assert len(set(np.array(t_mask).reshape(-1))) == 2;
        'error mask : ' + str(set(np.array(t_mask).reshape(-1)))
...
def make_dataset(site_name, train_size, val_size=0):
    ...
    if mode == 'train':
        image_folder = sorted(glob.glob(
            ('/data2/woans0104/Spinal_cord_dataset/{}/image/*'.format(site_name))))

        mask_folder = sorted(glob.glob(
            ('/data2/woans0104/Spinal_cord_dataset/{}/label/*'.format(site_name))))
